...
import { Calculator, Check, Download, Euro, Home, MapPinned, Sparkles, Video } from "lucide-react";
...
const APP_PRESETS = {
  base: {
    ...
    detail: "Overlay de base : avis Google, sections, points d’intérêt, menu et interface.",
    ids: ["web-app-immersive"],
  },
  full: {
    label: "Site immersif complet",
    detail: "App web + réservation, SEO/GEO, URL, automatisation, pop-up et carte personnalisée.",
...
function OptionCard({ active, icon: Icon, label, detail, price, onClick }) {
  return (
    <button type="button" onClick={onClick} className={`rounded-[24px] border-2 p-5 text-left transition ${pillClass(active)}`}>
      ...
      <div className="mx-auto max-w-6xl space-y-5 p-4 lg:p-6">
        <header className="overflow-hidden rounded-[30px] border border-slate-900 bg-slate-950 text-white shadow-2xl">
          <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-6 lg:p-8">
            ...
            <p className="mt-3 max-w-3xl text-base font-semibold leading-relaxed text-slate-300">
              Quelques informations suffisent pour obtenir une estimation simple : shooting, app web immersive et abonnement.
            </p>
          </div>
        </header>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <OptionCard active={wantsShooting} icon={MapPinned} label="Shooting 360° / 3D" detail="Captation intérieure et points extérieurs par Prodecta." price={eur(pricing.segments.shooting.setupPublic)} onClick={() => setWantsShooting((value) => !value)} />
          <OptionCard active={appPreset === "base"} icon={Sparkles} label={APP_PRESETS.base.label} detail={APP_PRESETS.base.detail} price="À partir de 600 €" onClick={() => setAppPreset("base")} />
          <OptionCard active={appPreset === "full"} icon={Home} label={APP_PRESETS.full.label} detail={APP_PRESETS.full.detail} price="2 130 €" onClick={() => setAppPreset("full")} />
        </section>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          {Object.entries(SUBSCRIPTION_PRESETS).map(([key, plan]) => {
            ...
            return <OptionCard key={key} active={active} icon={Euro} label={plan.label} detail={key === "essential" ? "Hébergement de l’app et de la visite virtuelle." : key === "growth" ? "Dashboard data, suivi des performances et re-shoot à -25 %." : "Rapports KPI, accompagnement stratégique et re-shoot à -50 %."} price={eur(price, " €/mois")} onClick={() => setSubscriptionPreset(key)} />;
          })}
        </section>

        <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-4 lg:grid-cols-[1fr_260px] lg:items-center">
            <button type="button" onClick={() => setWantsVideo((value) => !value)} className={`rounded-[24px] border-2 p-5 text-left transition ${pillClass(wantsVideo)}`}>
              <Video size={22} />
              <h3 className="mt-3 text-xl font-black">Vidéo IA / teaser 30 secondes</h3>
              <p className={`mt-2 text-sm font-semibold ${wantsVideo ? "text-emerald-50" : "text-slate-500"}`}>250 € par vidéo. Avec l’abonnement Premium, 100 € par vidéo.</p>
            </button>
