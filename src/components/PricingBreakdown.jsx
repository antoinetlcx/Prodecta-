    ...
    <div className={`grid gap-4 ${compact ? "2xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]" : "2xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]"}`}>
      <Card className={compact ? "rounded-2xl p-4 shadow-sm" : ""}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">HT</span>
        </div>
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              ...
              <tr className="bg-slate-950 text-white">
                ...
                <td className="px-4 py-4 text-xs font-bold text-slate-300">{pricing.selectedModules.length} modules</td>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Surface intérieure</p>
              <p className="mt-1 text-lg font-black">{pricing.intSurface} m²</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Coefficient</p>
              <p className="mt-1 text-lg font-black">{pricing.publicTier.coeff} €/m²</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Points extérieurs</p>
              <p className="mt-1 text-lg font-black">{pricing.points}</p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Prix par point</p>
              <p className="mt-1 text-lg font-black">{eur(pricing.unitPoint)}</p>
            </div>
          </div>
          <div className="rounded-2xl bg-emerald-50 p-4">
            <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Remise maximale setup</p>
